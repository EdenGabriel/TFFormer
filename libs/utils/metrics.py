...
# def analysis(input:pd.DataFrame):
#     # 计算每个label对应的平均时长
#     average_duration_by_label = input.groupby('label')['t-end'].mean() - input.groupby('label')['t-start'].mean()
    
#     durations = average_duration_by_label.round(2).to_list()
#     # 打印结果
#     print("average_duration_by_label",durations)
#     # 找到最大值和最小值
...
#     # 计算三个等分点
#     split_points = [min_duration + range_duration / 3,
#                     min_duration + (2 * range_duration) / 3]
#     split_points = [round(item, 2) for item in split_points]
#     print("split_points",split_points)
#     # 初始化存储索引的列表
#     short_duration,medium_duration,long_duration = [],[],[]
#     short_duration_indices,medium_duration_indices,long_duration_indices = [],[],[]

#     # 分割列表并记录索引
#     for i, duration in enumerate(durations):
#         if duration <= split_points[0]:
#             short_duration.append(duration)
#             short_duration_indices.append(i)
#         elif split_points[0] < duration <= split_points[1]:
#             medium_duration.append(duration)
#             medium_duration_indices.append(i)
#         else:
#             long_duration.append(duration)
#             long_duration_indices.append(i)                

#     # 打印结果
#     print("短时持续时间：", short_duration,short_duration_indices)
#     print("中时持续时间：", medium_duration,medium_duration_indices)
#     print("长时持续时间：", long_duration,long_duration_indices)



#     print("mean-->average_duration_by_label:",average_duration_by_label.mean())
#     print(average_duration_by_label.max(),average_duration_by_label.idxmax())
#     # 获取排序后的索引
#     sorted_indices = average_duration_by_label.argsort().tolist()
#     print("sorted_indices:",sorted_indices)
#     # 选择top k个索引（例如，选择前3个）
#     k = 1
#     top_k_indices = sorted_indices[-k:]
#     # 获取这些索引对应的值
#     top_k_values = [durations[index] for index in top_k_indices]

#     # 打印结果
#     print("---->Top ", k, "average_duration_by_label | indices:", top_k_indices,top_k_values)

#     return short_duration_indices,medium_duration_indices,long_duration_indices
...
def analysis(input:pd.DataFrame):
    ...
    short_duration_indices,long_duration_indices = [],[]
    ...
    return short_duration_indices,long_duration_indices
...
class ANETdetection(object):
    ...
    def __init__(
        ...
        self.short_didx,self.long_didx = analysis(self.ground_truth)
    ...
    def evaluate(self, preds, verbose=True):
        ...
        # ==============================
        data=np.array(self.ap)
        mean_value = np.mean(data,axis=0)
        print("mean values:",mean_value.tolist())

        result_list = [round(x * 100, 2) for x in mean_value.tolist()]
        print(result_list)

        # 获取排序后的索引
        # sorted_indices = np.argsort(mean_value)
        # # 选择top k个索引（例如，选择前3个）
        # k = 3
        # top_k_indices = sorted_indices[-k:]
        # # 获取这些索引对应的值
        # top_k_values = mean_value[top_k_indices]
        # # 打印结果
        # print("Top", k, "indices:", top_k_indices,top_k_values)
        ...
        if verbose:
            ...
            # =================
            print('[RESULTS] Action detection results on actions of different length.')

            short_ap = get_idx_value(self.ap,self.short_didx,round=4)
            long_ap = get_idx_value(self.ap,self.long_didx,round=4)
            print("\033[1;95m ---------------Short Actions---------------\033[0m")
            get_length_map(self.tiou_thresholds,short_ap)
            print("\033[1;95m ---------------Long Actions---------------\033[0m")
            get_length_map(self.tiou_thresholds,long_ap)
            
        return mAP, average_mAP, mRecall

def get_idx_value(input,idx_list,round=2):
    # 使用嵌套的列表推导式保留所有值为两位小数
    input = np.round(np.array(input),round).tolist()
    res = []
    for tiou_thre_list in input:
        extracted_sublist = [tiou_thre_list[index] for index in idx_list]
        res.append(extracted_sublist)
    return res

def get_length_map(tiou_threshold,map_list):
    map_list = np.array(map_list).mean(axis=1).tolist() 
    average_mAP = np.array(map_list).mean()
    block = ''
    for tiou, tiou_mAP in zip(tiou_threshold, map_list):
        block += '\n|tIoU = {:.2f}: '.format(tiou)
        block += 'mAP = {:>4.2f} (%) '.format(tiou_mAP*100)
    print(block)
    print('Average mAP: {:>4.2f} (%)'.format(average_mAP*100))
