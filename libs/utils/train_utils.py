...
def make_optimizer(model, optimizer_config):
    ...
    whitelist_weight_modules = (
        torch.nn.Linear, torch.nn.Conv1d, MaskedConv1D,)
    blacklist_weight_modules = (
        LayerNorm, torch.nn.GroupNorm)
    ...
    for mn, m in model.named_modules():
        for pn, p in m.named_parameters():
            ...
            if pn.endswith('bias'):
                ...
                decay.add(fpn)
            # elif pn.endswith('factor'):
            #     decay.add(fpn)
            # elif pn.endswith('0') or pn.endswith('1') or pn.endswith('2') or pn.endswith('3') or pn.endswith('4') or pn.endswith('5'):
            #     no_decay.add(fpn)
            # elif pn.endswith('gates'):
            #     no_decay.add(fpn)
...
def valid_one_epoch(
    ...
    if evaluator is not None:
        ...
        convert_dict_to_json(output_file)
    else:
