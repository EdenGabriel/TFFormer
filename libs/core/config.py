...
DEFAULTS = {
    ...
    "model": {
        ...
        "embd_kernel_size": 3,
        # output dim of embedding corresponding to the input dim of Neck
        "embd_dim": 768,  # 768
        # if attach group norm to embedding network
        "embd_with_ln": True,
        # ? output dim of Neck corresponding to the input dim of head
        # ? if fpn_type == identity, fpn_dim need be equal to embd_dim
        # ? if fpn_type == fpn, fpn_dim can be not equal to embd_dim
        # feat dim for FPN, ----------------- input dim for head
        "fpn_dim": 768,  # 768 (identity)
        # if add ln at the end of fpn outputs
        "fpn_with_ln": True,
        # starting level for fpn
        "fpn_start_level": 0,
        # feat dim for head， ----------------- output dim for head
        "head_dim": 512,
